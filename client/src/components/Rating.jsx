...
const Rating = ({ productId, setFilter }) => {
  ...
  useEffect(() => {
    ...
    const processReviews = (reviews) => {
      ...
      const averageRating = total > 0 
        ? (Object.keys(distribution).reduce((sum, key) => sum + key * distribution[key], 0) / total).toFixed(1)
        : 0;
          ...
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, index) => (
              <img
                ...
                src={star}
                alt="star"
                className={`w-6 h-6 ${index < Math.round(rating) ? 'opacity-100' : 'opacity-30'}`}
              />
            ))}
          </div>
          <div className="font-bold text-primarymain text-base text-center">
        ...
        <div className="flex flex-col w-[333px] gap-2.5"> 
          ...
          <div className="flex flex-wrap items-center gap-5"> 
            {['Semua', '5', '4', '3', '2', '1'].map((filter, index) => ( 
              <div 
                key={index} 
                className="flex items-center justify-center gap-3 px-4 py-2 rounded-full border border-primarymain cursor-pointer"
